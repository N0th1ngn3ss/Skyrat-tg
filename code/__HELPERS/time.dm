...
/proc/DisplayTimeText(time_value, round_seconds_to = 0.1)
	...
	if(!second)
		return "ahora mismo"
	if(second < 60)
		return "[second] segundo[(second != 1)? "s":""]"
	var/minute = FLOOR(second / 60, 1)
	second = FLOOR(MODULUS(second, 60), round_seconds_to)
	var/secondT
	if(second)
		secondT = " y [second] segundo[(second != 1)? "s":""]"
	if(minute < 60)
		return "[minute] minuto[(minute != 1)? "s":""][secondT]"
	var/hour = FLOOR(minute / 60, 1)
	minute = MODULUS(minute, 60)
	var/minuteT
	if(minute)
		minuteT = " y [minute] minuto[(minute != 1)? "s":""]"
	if(hour < 24)
		return "[hour] hora[(hour != 1)? "s":""][minuteT][secondT]"
